...
@router.get("/players/", response_model=List[PlayersSchema] | PlayersSchema)
async def get_players(db: AsyncSession = Depends(get_session), team_id: Optional[str] = None):
    if team_id:
        result = await db.execute(select(PlayersModel).filter(PlayersModel.team_id == team_id))
    else:
